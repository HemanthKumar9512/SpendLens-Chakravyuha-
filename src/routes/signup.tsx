  ...
  const onSubmit = async (data: Form) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: { org_name: data.org_name, full_name: data.full_name },
      },
    });
    setLoading(false);
    if (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[color:var(--navy)] via-[color:var(--navy)] to-[color:var(--teal)]/30 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-2 text-center">
          ...
          <CardDescription>Start governing your cloud spend</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            <div>
              <Label htmlFor="org">Organisation</Label>
              <Input id="org" {...register("org_name")} />
              {errors.org_name && <p className="text-xs text-destructive mt-1">{errors.org_name.message}</p>}
            </div>
            <div>
